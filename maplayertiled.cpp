...
void MapLayerTiled::Initialize(u32 tilesWidth, u32 tilesHeight)
{
	...
	iMapWidth = iTileSize * iTilesWidth;
	iMapHeight = iTileSize * iTilesHeight;
	iViewWidth = iTileSize * iViewTilesWidth;
	iViewHeight = iTileSize * iViewTilesHeight;
...
void MapLayerTiled::SetView(u32 viewWidth, u32 viewHeight)
{
	...
	f32 sptX = static_cast<f32>(iTileSize) / static_cast<f32>(pScreen->GetWidth());
	f32 sptY = static_cast<f32>(iTileSize) / static_cast<f32>(pScreen->GetHeight());
...
Point2i MapLayerTiled::ViewAt(Point2i pos)
{
	...
	else
	{
		...
		if (x > xd + (s32)iTileSize - 1)
			x = xd + iTileSize - 1;

		if (y > yd + (s32)iTileSize - 1)
			y = yd + iTileSize - 1;
	}

	Point2i ret(x, y);

	s32 newX = x / static_cast<s32>(iTileSize);
	s32 newY = y / static_cast<s32>(iTileSize);

	iOffsetX = x - (newX * iTileSize);//x & (iTileSize - 1);
	iOffsetY = y - (newY * iTileSize);//y & (iTileSize - 1);
...
void MapLayerTiled::SetTileSize(u32 size)
{
	iTileSize = size;
}
